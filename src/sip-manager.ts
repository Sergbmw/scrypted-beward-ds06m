...
import { decodeSrtpOptions, timeoutPromise } from './compat'
...
function parseRtpDescription(console: any, inviteResponse: {
  ...
  const sections: string[] = sdp.splitSections(inviteResponse.content),
    lines: string[] = sections.flatMap((section: string) => sdp.splitLines(section)),
    cLine = lines.find((line: string) => line.startsWith('c='))!

  if (!cLine) {
    console.error('DS06M SDP has no connection line:', inviteResponse.content || 'empty');
    throw new Error('DS06M SDP has no connection line');
  }
...
export class SipManager {
  ...
  private incomingInviteCallId: string
  ...
  public sendResponse(request: SipRequest, status: number, reason: string) {
    const response = this.sipStack.makeResponse(request, status, reason)
    if (this.toParams.tag) {
      response.headers.to.params.tag = this.toParams.tag
    }
    if (request.headers["record-route"]) {
      response.headers["record-route"] = request.headers["record-route"]
    }
        ...
        (request: SipRequest) => {
          if (request.method === 'BYE') {
            ...
            //this.sipStack.send(tryingResponse)
              // Retransmitted INVITEs keep the current call identifier and tag.
              let ringResponse = this.sipStack.makeResponse(request, 180, 'Ringing')
              const incomingCallId = request.headers['call-id']
              if (this.incomingInviteCallId !== incomingCallId) {
                this.incomingInviteCallId = incomingCallId
  ...
  */
  async invite( rtpOptions : RtpOptions, audioSection, videoSection?, incomingCallRequest? ) : Promise<RtpDescription> {
    ...
    if( incomingCallRequest ) {
      // Keep the same local dialog tag that was advertised in the 180
      // Ringing response. Some panels (including DS06M) will display the
      // call but ignore the answered media dialog if the 200 OK changes or
      // omits that tag.
      const localTag = this.toParams.tag || getRandomId()
      const remoteTag = incomingCallRequest.headers.from.params?.tag
      ...
      callResponse.headers.to.params = callResponse.headers.to.params || {}
      callResponse.headers.to.params.tag = localTag
      if( incomingCallRequest.headers["record-route"] )
        callResponse.headers["record-route"] = incomingCallRequest.headers["record-route"];
      const contactUri = this.sipOptions.domain
        ? (from.split('@')[0] + '@' + this.sipOptions.domain).trim()
        : from
      callResponse.headers.contact = [{ uri: contactUri }]

      // For requests that Scrypted sends inside this incoming dialog (BYE,
      // INFO), From uses our local tag and To uses the panel's remote tag.
      this.fromParams.tag = localTag
      this.toParams.tag = remoteTag
